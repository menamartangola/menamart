    ...
    <div className={`prod-card${product.stock?"":" out-of-stock"}`}>
      ...
      {inCart ? (
        <div className="qty-ctrl">
          ...
          <input type="number" min="1" value={cartItem.qty} onChange={e=>{const v=parseInt(e.target.value);if(!isNaN(v)&&v>0)onChangeQty(product.id,v-cartItem.qty);}} style={{width:40,textAlign:"center",border:"1px solid var(--border)",borderRadius:6,fontFamily:"var(--font-body)",fontWeight:800,fontSize:14,padding:"2px 0",outline:"none"}} />
          <button className="qty-btn" onClick={()=>onChangeQty(product.id,1)}>+</button>
        </div>
      ) : (
